...
|-------|--------------------|----------|
| Domain (pure logic) | `DisplayDomain`, `ProviderInterfaces`, `SceneEngine`, `AutomationSchema`, `TopologyCore`, `SimulatorProvider` | cross-platform; verified locally with `make test` |
...
- **Public-API-only** — documented Apple APIs + hardware/network protocols only; no
  private lifecycle/virtual/system-override provider. Keep this flavor compiling locally (NFR-010).
